        ...
        public static IDictionary<string, ResourceSchema> Parse(CodeModel serviceClient, string version)
        {
            ...
            foreach (Method method in serviceClient.Methods.Where( method => method.Parameters.FirstOrDefault(p => p.SerializedName == "api-version")?.DefaultValue.Value == version || version == serviceClient.ApiVersion))
            {
                ...
                foreach (string resourceType in resourceTypes)
                {
                    ...
                    resourceDefinition.AddProperty("type", JsonSchema.CreateSingleValuedEnum(resourceType), true);
                    resourceDefinition.AddProperty("apiVersion", JsonSchema.CreateSingleValuedEnum(apiVersion), true);
                    ...
                    if (method.Body != null)
                    {
                        ...
                        if (body != null)
                        {
                            ...
                            HandlePolymorphicType(resourceDefinition, body, resourceSchema.Definitions, serviceClient.ModelTypes);
                        }
                    }
